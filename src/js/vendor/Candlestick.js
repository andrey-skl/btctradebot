...
window.Candlestick = function(canvasID, rawData, options){
  ...
  function convertYahooFinanceCsvToCandles(rawData) {
    ...
    for(var i=rawData.length-1; i>0; i--){
      var row = rawData[i];
      d.push(row.date);
      o.push(Number((row.open*ratio).toFixed(2)));
      h.push(Number((row.max*ratio).toFixed(2)));
      l.push(Number((row.min*ratio).toFixed(2)));
      c.push(Number((row.close*ratio).toFixed(2)));
      v.push(Number((row.volume/ratio).toFixed(0)));
    }
